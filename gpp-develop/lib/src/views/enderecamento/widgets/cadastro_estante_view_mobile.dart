...
// ignore: must_be_immutable
class CadastroEstanteViewMobile extends StatefulWidget {
  String? idCorredor;
  CadastroEstanteViewMobile({this.idCorredor});
...
class _CadastroEstanteViewState extends State<CadastroEstanteViewMobile> {
  ...
  buscarTodos(String idCorredor) async {
    try {
      enderecamentoController.listaEstante = await enderecamentoController.repository.buscarEstante(int.parse(idCorredor));
      if (mounted) {
        setState(() {
  ...
  handleDelete(context, EstanteEnderecamentoModel estanteEnderecamentoModel) async {
    try {
      if (await Notificacao.confirmacao("Você deseja excluir a estante?")) {
        if (await enderecamentoController.excluirEstante(widget.idCorredor.toString(), estanteEnderecamentoModel)) {
          ...
          builder: (context, setState) {
            return AlertDialog(
                title: Row(
                  ...
                  children: [
                    const Text("Cadastro da Estante"),
                  ],
                ),
                                ...
                                !loading
                                    ? ButtonComponent(
                                        ...
                                        onPressed: () async {
                                          ...
                                          await handleCreate(
                                              context, enderecamentoController.estanteModel, widget.idCorredor.toString());
                                        },
                                        text: 'Adicionar',
                                      )
                  ...
                  children: [
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          const Flexible(child: const TitleComponent('Estantes')),
                          ButtonComponent(
                              onPressed: () {
                                ...
                                return Container(
                                  ...
                                  child: CardWidget(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          const Expanded(child: const TextComponent('Estante', fontWeight: FontWeight.bold)),
                                          const Expanded(child: const TextComponent('Ações', fontWeight: FontWeight.bold)),
                                        ],
                                      ),
                                      const Divider(
                                        thickness: 1,
                                        ...
                                        children: [
                                          Expanded(
                                            child: SelectableText(
                                                enderecamentoController.listaEstante[index].desc_estante.toString()),
                                          ),
                                          Expanded(
                                            child: Row(
                                              children: [
